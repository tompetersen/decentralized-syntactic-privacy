...
def main():
    path_runs_motion = "./ccblade_bak/runs_qid_motion/*.txt"
    path_runs_securesum = "./ccblade_bak/runs_qid_securesum/*.txt"

    motion_runs = defaultdict(dict)
    for file in glob.glob(path_runs_motion):
        run = evaluate(file)
        motion_runs[run["dataset"]][run["num_qids"]] = run

    securesum_runs = defaultdict(dict)
    for file in glob.glob(path_runs_securesum):
        run = evaluate(file)
        securesum_runs[run["dataset"]][run["num_qids"]] = run
    ...
    methods = ["motion" for _ in range(len(motion_runs) * len(motion_runs["adult"]))] + ["securesum" for _ in range(len(securesum_runs) * len(securesum_runs["adult"]))]
    ...
    df.sort_values(by=["dataset", "method", "rounds"], inplace=True)
    ...
    sns.lineplot(data=df, x="num_qids", y="time", hue="method", ax=axs)
    fig.savefig("img/qid_adult_time.png")
    ...
    fig, axs = plt.subplots()
    sns.lineplot(data=df, x="num_qids", y="bytes", hue="method", ax=axs)
    fig.savefig("img/qid_adult_bytes.png")
